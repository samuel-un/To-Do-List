// Configuración y obtención de elementos del DOM
...
function renderTodos(filteredTodos = todos) {
    ...
    filteredTodos.forEach(todo => {
        const formattedDate = todo.date ? new Date(todo.date).toLocaleDateString("es-ES") : "Sin fecha";
...
function sortByDate() {
    const sorted = [...todos].sort((a, b) => {
        const dateA = a.date ? new Date(a.date) : new Date(0); // Fecha predeterminada para tareas sin fecha
...
function toggleComplete(id) {
    ...
    if (todo) {
        Swal.fire({
            title: "¿Estás seguro?",
            text: `¿Quieres marcar esta tarea como "${todo.completed ? "pendiente" : "completada"}"?`,
            icon: "question",
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire(
                    "¡Actualizado!",
                    `La tarea ha sido marcada como "${todo.completed ? "completada" : "pendiente"}".`,
                    "success"
                );
            }
        });
